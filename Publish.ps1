...
# Progress output goes through Write-Information so a caller can redirect, capture or suppress it,
# which output written straight to the host cannot be. -InformationAction Continue keeps it visible
# by default, which is what this script wants when run interactively.
function Write-Status {
	...
	$text = if ($Colour -eq 'Default' -or -not $PSStyle) {
		$Message
	}
	else {
		"$($PSStyle.Foreground.$Colour)$Message$($PSStyle.Reset)"
	}
